class StringsManager {
  static String titleOnBoardingPage = r'''Welcome to NFT
 Marketplace''';
  static const String titleCardOnBoardingPage = "Explore and Mini NFTs";
  static const String descriptionCardOnBoardingPage =
      "You can buy and sell the NFTs of the best artist in the world";
  static const String btnTitleCardOnBoardingPage = "Get Stareted Now";
  static const String titleHomepage = "NFT Marketplace";
  static const String trendingCollectionHomepage = 'Trending Collection';
  static const String topSellerCollectionHomepage1 = 'Top Seller';
  static const String StatsTitle = 'Stats';
  static const String ranking = 'Ranking';
  static const String activity = 'Activity';
}
